...
describe('CbusProjectParser', () => {
    ...
    describe('parseXML', () => {
        ...
        it('rejects XML longer than maxDecompressedBytes', async () => {
            const tiny = new CbusProjectParser({ maxDecompressedBytes: 50 });
            const xml = '<?xml version="1.0"?><Network>' + 'x'.repeat(100) + '</Network>';
        ...
        it('rejects XML with an excessive element-token count', async () => {
            // MAX_XML_ELEMENT_TOKENS is 200000; <?xml + <r> + N*<i/> + </r> => N+3 tokens.
            const xml = '<?xml version="1.0"?><r>' + '<i/>'.repeat(200000) + '</r>';
            await expect(parser.parseXML(xml))
