...
class SerialDeviceRecovery {
    ...
    /**
     * A numeric setting with a hard floor.
     *
     ...
     * loop. Non-finite values fall back to the schema default, then the floor
     * is applied so 0 (never sensible for these) becomes the floor rather than
     * disabling the bound.
     *
     * @param {string} key
     * @param {number} floor
    ...
    _clampedSetting(key, floor) {
        const resolved = Number(resolveSetting(this.settings, key));
        const value = Number.isFinite(resolved)
            ? resolved
            : Number(resolveSetting({}, key));
